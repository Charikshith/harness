...
| [pre-commit.sh](pre-commit.sh) | template | .githooks/pre-commit | git, hook, enforcement, commit, lifecycle | Blocks a commit that changes non-harness files without also updating harness/progress.md and harness/memory/journal.md. Optional; auto-scaffolded, activated by init.sh setting core.hooksPath |

...
> never said where the observation should go, so it went into `progress.md` prose that
...
has encountered in the code. Features belong in `feature_list.json`; curation proposals
belong in `dream-queue.md`.

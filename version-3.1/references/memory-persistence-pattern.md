...
- **Auto-memory** (agent-written, persistent): Discovered patterns, preferences, pitfalls — the `memory/` store
- **State artifacts** (agent-written, episodic): Progress logs, session handoffs, plans
- **Session extraction** (background-derived): Automatic transcript analysis at session end

> **State is not memory.** Progress logs and session handoffs answer *"where did the work
> stop?"* — a bookmark, true only until the current feature ships. Memory answers *"what
> is now known that wasn't before?"* — a lesson that outlives the feature. Both are
> agent-written and persistent, which is why they get conflated, but the harness scores
> them as separate subsystems (`state` and `memory`) because a harness can track state
> perfectly and never learn anything. The practical test: if the note stops being true
> when the current feature ships, it is state. See
> [Dreaming](dreaming-pattern.md) for how the memory half is curated.
...
6. **Build review mechanism** for promotion proposals — see [Dreaming](dreaming-pattern.md)
   for the batch curation loop, its cadence, and the propose-never-apply gate
